...
%--------------------------------------------------------------------------
% remove path: close all; clear all; path(pathdef); clc;
%-------------------------------------------------------------------------
...
%                 Algorithm = ALG(input{:}); %% KONG-CHANGE
            else
                Algorithm = ALG(input{:});
            end
...
function [name,input] = getSetting(setting,Pro)
    ...
    if nargin > 1
        ...
        keys = {'save','outputFcn'}; %% KONG-CHANGE
%         keys = {};
    else
    ...
    input = [input,setting(reshape([index;index+1],1,[]))]; %% KONG-CHANGE
end